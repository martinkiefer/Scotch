#Scaling script for Column Sketches with Select-Update
cwd=$(pwd)
PROJECT_FOLDER="$cwd/../.."
GENERATOR="$PROJECT_FOLDER/ScotchDSL/Translation/ReplicatedColumnSketchGenerator.py"
...
N_REPS=1
DESCRIPTOR="$PROJECT_FOLDER/Sketches/Select-Update/AGMS/descriptor.json"
...
while true; do
    ...
    for SEED in {1..5}; do
        ...
        $PYTHON $GENERATOR $DESCRIPTOR $ROWS $DISPATCH_FACTOR $COLLECT_FACTOR $N_REPS $FOLDER/template/sketch
        cd $FOLDER/template
