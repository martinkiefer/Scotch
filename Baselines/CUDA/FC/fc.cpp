...
{    
    ...
    for(unsigned int i = 0; i < p->replicas; i++){
	unsigned int j = id*(partition_size);
	    ...
	    v16ul in;
            ...
            for(unsigned int k = 0; k < 8; k++){
                p->sk_t0[id*p->replicas*p->skn_cols + i*p->skn_cols + select[k]] += 1;
            }
        }
        ...
        for(; j < partition_upper && j < p->ts0; j++){
            ...
            select = select % p->skn_cols;
            p->sk_t0[id*p->replicas*p->skn_cols + i*p->skn_cols + select] += 1; 
        }
...
int main( int argc, const char* argv[] )
{
    ...
    int replicas = atoi(argv[1]);
    int skn_cols = atoi(argv[2]);
    p.nthreads = atoi(argv[3]);
    p.replicas = replicas;
    p.skn_cols = skn_cols;
    //std::cout << "Rows: " << p.replicas << " x " << p.skn_cols << std::endl;
    
    p.ts0= 268435456*2;

    p.sk_t0 = (int*) aligned_alloc(64,p.replicas*p.skn_cols*p.nthreads*sizeof(int));
    std::memset(p.sk_t0, 0, p.replicas*p.skn_cols*p.nthreads*sizeof(int));
    ...
    //std::cout << "Normalized execution time: " << ((float) time /p.replicas) << std::endl;
    //std::cout << "Throughput: "<< p.ts0*sizeof(unsigned int)*8.0 / (1000.0*1000.0*1000.0*time/1000.0) << " gbps" << std::endl;
    //std::cout << "Normalized Throughput: "<< p.ts0*sizeof(unsigned int)*8.0*p.replicas / (1000.0*1000.0*1000.0*time/1000.0) << " gbps" << std::endl;
    std::cout << p.replicas << ";" << p.skn_cols << ";" << p.ts0*sizeof(unsigned int)*8.0 / (1000.0*1000.0*1000.0*time/1000.0) << std::endl;

    writeSArrayToFile("sketch.dump", p.sk_t0, p.replicas*p.skn_cols*p.nthreads);
}
